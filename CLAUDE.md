...
**`index.html`'s mobile redirect** (top of `<head>`) forwards the query string — `location.replace('mobile.html' + location.search)` instead of a bare `'mobile.html'` — so a `?case=` link isn't dropped on a narrow viewport. That link now actually works on the other side: **`mobile.html` is wired into this case-overlay system** (this section previously said it wasn't — that description was stale, corrected 2026-09-25 against the actual files). Specifically:

- `mobile.html` loads `case-overlay.js` like the other host pages.
- Its DrayEasy button (`mobile.html:96`) carries `data-case-study="drayeasy"` with `href="drayeasy.html"` as the progressive-enhancement fallback — the same two-part setup as `content.html`'s button, not a separate implementation. It is no longer `data-case-study-ready="false"`, so `mobile.js`'s own Coming Soon modal no longer matches it.
- `buildCaseUrl()` derives the URL from `location.pathname`, so opening a case study on mobile syncs to `mobile.html?case=<key>`, and landing on that URL directly auto-opens the overlay via the same `syncFromLocation()` path. No mobile-specific branch.
- `case-overlay.css` has a real ≤1023px treatment (panel goes fullscreen `inset: 0` and slides up from the bottom instead of the desktop 40px-inset panel), and the tap-to-zoom lightbox for large images is **mobile-only** (`≤1023px`, `[data-lightbox]`).

Still pending on mobile: MobileConductor (`mobile.html:182`) and Beyond Fingers (`mobile.html:219`) remain `data-case-study-ready="false"`, and `drayeasy.css` itself has not had a full mobile responsive pass.
...
- **GA4 analytics** (`analytics.js`, GA ID `G-E0R93HND7S`): all four HTML pages load it via a single `<script src="analytics.js">` tag; it owns the gtag.js bootstrap and exposes `window.trackEvent(name, params)` as the one function everything else should call (never `gtag()` directly) — safe to call even if GA is blocked or the page is `localhost`/`127.0.0.1` (bootstrap is skipped entirely on those hosts, not just the custom events, since GA4's own `config` call fires an automatic `page_view`). Events wired so far: `open_section`/`view_section` (`overlay.js`, first-layer sections), `view_about_item` (`overlay.js` + `content.js`'s `initAboutItemAnalytics`, the 4 About cards via their `data-about-item` attribute), `open_case_study`/`case_study_scroll` (`case-overlay.js`), `click_resume` (delegated inside `analytics.js` itself, matches `a[aria-label="Resume"]`).
  - **Known inaccuracy, not yet fixed**: `open_section`'s `method` param is hardcoded `'click'` in `overlay.js`'s `openOverlay(key)`, but that function is also called programmatically now (`case-overlay.js`'s `syncFromLocation()` calls `window.openContentOverlay('work')` when landing on `index.html?case=<key>` — see §2.2) — those opens get misreported as `method:'click'` too. Fix would be adding a second parameter to `openOverlay` so callers can pass the real origin (`'click'` vs something like `'direct_link'`) — deliberately not done yet since it touches `openOverlay`'s signature, not just an added call. Revisit once there are more case studies and shared/deep-link traffic through this path is actually worth the accuracy.
  - `mobile.html` has no equivalent IntersectionObserver-based section/about-item tracking — it's a single flat scrolling page with no overlay system, so `initSectionAnalytics`/`initAboutItemAnalytics` (which key off `overlay.js`'s open/close lifecycle) don't apply there as-is. Deliberately deferred, to be done alongside/after the mobile responsive pass on `drayeasy.css` (see the Mobile/responsive bullet below).
